...
public class TileEntityMover extends TileEntityJPT implements TileEntityJPT.EnergyReceiver, ITickable {
    ...
    public void update() {
        ...
        BlockPos me = getPos();
        EnumFacing facing = JAPTA.safeGetValue(getWorld().getBlockState(me), BlockMover.FACING);
        transmit(facing);
        BlockPos front = me.offset(facing);
        List<Entity> l = getWorld().getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(me.getX(), me.getY() + 1, me.getZ(), me.getX() + 1, me.getY() + 2, me.getZ() + 1));
        l.addAll(getWorld().getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(front.getX(), front.getY(), front.getZ(), front.getX() + 1, front.getY() + 1, front.getZ() + 1)));
        for (Entity e : l) {
